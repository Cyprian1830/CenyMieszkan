...
# Zmienna integer czyli arg1, nie zmieniła swojej wartości gdzyż jest inmutable, a
...
def zamowienie_produktu(nazwa_produktu, *, cena, ilosc=1):
    tekst = str(f"nazwa produktu: {nazwa_produktu}, cena: {cena*ilosc}, ilość: {ilosc}")
    wartosc_zamowienia = cena*ilosc
    return tekst, wartosc_zamowienia
...
def stworz_raport(*args, **kwargs):
    ...
    for i in range(dlugosc):
            id = args[i]
            print()
            raport += str(f"ID: {id}, nazwa: {kwargs[f'nazwa_{id}']}, cena: {kwargs[f'cena_{id}']}\n")
    return raport


print(stworz_raport(101, 102, nazwa_101="Kubek termiczny",
                    cena_101="45.99 zł", nazwa_102="Długopis", cena_102="4.99 zł"))
